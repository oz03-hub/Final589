# CHANGELOG from HW1:
# - Added numeric data handling
# - Added multiprocessing
# - Added new stopping criteria
# - Refactored code to be in a class format like sklearn api and encapsulation mostly
# - Bulk of the algorithm is the same as HW1
...
class NumericDecisionNode:
    def __init__(self, attribute: str, split_point: float, edges: dict, majority_class: str):
...
class RandomDecisionTree:
    def __init__(self, bagged_data: pd.DataFrame, m=None, stopping_criterion="minimal_gain", stopping_criterion_hyperparameter=0.01, class_column="label", numeric_end="_num"):
        ...
        self.attributes: list[str] = bagged_data.columns.drop(self.class_column).to_list()
        if m is None:
        ...
        self.stopping_criterion_hyperparameter: float = stopping_criterion_hyperparameter
    ...
    def _id3(self, df: pd.DataFrame, attribute_to_split: str) -> tuple[any, float]:
        ...
        if attribute_to_split.endswith(self.numeric_end): # numeric attribute
            ...
            avg_entropy = (len(df_left) / len(df)) * e_left + (len(df_right) / len(df)) * e_right
            gain = og_entropy - avg_entropy
            return split_point, gain
        else: # categorical attribute
            # HW1 Code
            attribute_values = np.unique(df[attribute_to_split])
            splits = {value: df[df[attribute_to_split] == value] for value in attribute_values}
            avg_entropy = sum(len(split) / len(df) * self._entropy(split[self.class_column]) for split in splits.values())
    ...
    def _fit_recursively(self, df: pd.DataFrame, depth: int = 0):
        ...
        if all(df[self.class_column] == df[self.class_column].iloc[0]):
            return LeafNode(df[self.class_column].iloc[0]) # Create Leaf node with label
        ...
        if self.stopping_criterion == "maximal_depth" and depth >= self.stopping_criterion_hyperparameter:
            return LeafNode(majority_class)
        
        # BASE CASE: minimal size for split
        if self.stopping_criterion == "minimal_size_for_split" and len(df) < self.stopping_criterion_hyperparameter:
            return LeafNode(majority_class)
        ...
        attribute_gains: dict[str, tuple[any, float]] = {attribute: self._id3(df, attribute) for attribute in attributes_to_consider}
        best_gain_entry: tuple[str, tuple[any, float]] = max(attribute_gains.items(), key=lambda x: x[1][1]) # find max gain
        ...
        if self.stopping_criterion == "minimal_gain" and best_gain < self.stopping_criterion_hyperparameter:
            return LeafNode(majority_class)
...
class RandomForest:
    def __init__(self, n_tree: int, data: pd.DataFrame, class_column: str = "label", m: int = None, stopping_criterion: str = "minimal_gain", stopping_criterion_hyperparameter: float = 0.01, numeric_end: str = "_num", multiprocessing: bool = True):
        """stopping_criterion can be minimal_gain, minimal_size_for_split, or maximal_depth"""
        self.n_tree = n_tree
        self.data = data
    ...
    def _fit(self):
        ...
        if self.multiprocessing:
            ...
            with mp.Pool(processes=mp.cpu_count() - 2) as pool: # it will try not to abuse your cpu
    ...
    def _fit_tree(self, boot: pd.DataFrame):
        return RandomDecisionTree(boot, m=self.m, stopping_criterion=self.stopping_criterion, stopping_criterion_hyperparameter=self.stopping_criterion_hyperparameter, numeric_end=self.numeric_end)

    def _create_boots(self):
        boots = []
        for _ in range(self.n_tree):
            boot = self.data.sample(n=len(self.data), replace=True) # bootstrap, with replacement
            boots.append(boot)
        return boots
    ...
    def predict(self, x: pd.Series):
        ...
        if len(counts.items()) > 1 and counts.most_common(1)[0][1] == counts.most_common(2)[1][1]:
